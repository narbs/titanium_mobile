...
{
  DebugLog(@"eventStoreChanged...");
  if ([self _hasListeners:@"change"]) {
    DebugLog(@"eventStoreChanged, firing change event...");
...
- (NSArray<TiCalendarCalendar *> *)allEditableCalendars
{
  ...
  if (![NSThread isMainThread]) {
    DebugLog(@"allEditableCalendars, not main thread path...");
    __block id result = nil;
    TiThreadPerformOnMainThread(
        ^{
  ...
  TiThreadPerformOnMainThread(
      ^() {
        ...
        if (@available(iOS 17.0, *)) {
          ...
          NSLog(@"[INFO] Calendar requestAuthorization iOS 17...");
          ...
          NSLog(@"[INFO] Calendar requestAuthorization iOS 16...");
